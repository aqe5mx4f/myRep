...
const fs = require('fs');
const headercourse = require("../../db/headercourse");
var courseSummList, headercourseList;
console.log("register");
var file = path.join(__dirname, 'lessonDetail.json');
fs.readFile(file, 'utf-8', function(err, data) {
    if (data) {
        console.log('读取lessonDetail成功');
        courseSummList = JSON.parse(data);
    }
});

var file_headercourse = path.join(__dirname, 'headercourse.json');
fs.readFile(file_headercourse, 'utf-8', function(err, data) {
    if (data) {
        console.log('读取headercourse成功');
        headercourseList = JSON.parse(data);
    }
});
...
router.post('/', (req, res) => {
    ...
    function createDatabase() {
        ...
        var obj = [
            ...
            [ //one
                [
                    { name: '人工智能导论', school: '浙江工业大学', teacher: '王万良', progress: 2, end: false, hot: 140, img: 'ArtiInte_zeda.webp' },
                    { name: '物联网工程导论', school: '哈尔滨工程大学', teacher: '刘海波', progress: 2, end: false, hot: 100, img: 'wuliwang_harbing.webp' },
                    { name: '大数据技术原理与应用', school: '厦门大学', teacher: '林子雨', progress: 2, end: false, hot: 160, img: 'LargeData_xiada.webp' },
                    { name: '大数据算法', school: '哈尔滨工业大学', teacher: '王宏志', progress: 2, end: false, hot: 100, img: 'LargeDatasuanhua.webp' }
                ],
                [
                    { name: 'Python云端系统开发入门', school: '北京理工大学', teacher: '嵩天', progress: 2, end: false, hot: 140, img: 'PythonCloudSysOpen_beili.webp' },
                    { name: 'Python游戏开发入门', school: '北京理工大学', teacher: '嵩天', progress: 2, end: false, hot: 100, img: 'PythonGameOpen_beili.webp' },
                    { name: 'Python机器学习应用', school: '北京理工大学', teacher: '礼欣', progress: 2, end: false, hot: 160, img: 'PythonMachStudy_beili.webp' }
                ]
            ],
            [ //two
                [
                    { name: '大学英语（口语）CAP', school: '国防科技大学', teacher: '彭天笑', progress: 2, end: false, hot: 140, img: 'CollOralEng_guokeda.webp' },
                    { name: '表达与沟通：海外留学英语', school: '浙江大学', teacher: '方凡', progress: 2, end: false, hot: 100, img: 'ShowSelfCommui_zeda.webp' },
                    { name: '国际交流英文演讲与辩论', school: '湖南大学', teacher: '邓媛', progress: 2, end: false, hot: 160, img: 'EngSpeeDiscuss_huda.webp' }
                ],
                [
                    { name: '英语语法与句子写作', school: '集美大学', teacher: '赵以', progress: 2, end: false, hot: 140, img: 'GrammarSentenceWrite_jimei.webp' },
                    { name: '英语语法与写作', school: '暨南大学', teacher: '朱湘军', progress: 2, end: false, hot: 100, img: 'GrammarArticleWrite_jinan.webp' },
                    { name: '新大学英语·阅读与思辨（上）', school: '重庆大学', teacher: '李小辉', progress: 2, end: false, hot: 160, img: 'NewCollEng_chongqing.webp' }
                ]
            ],
            [ //three
                [
                    { name: '国际经济学', school: '首都经济贸易大学', teacher: '李婧', progress: 2, end: false, hot: 140, img: 'InterEcon_shoudujm.webp' },
                    { name: '高等数学典型例题与解法（二）', school: '国防科技大学', teacher: '李建平', progress: 2, end: false, hot: 100, img: 'AdvMathlitijiefa_guokeda.webp' }
                ],
                [
                    { name: '电磁学', school: '南京大学', teacher: '吴小山', progress: 2, end: false, hot: 140, img: 'ElectMagn_nanjingda.webp' },
                    { name: '大学物理—力学、振动与波动', school: '大连理工大学', teacher: '李雪春', progress: 2, end: false, hot: 100, img: 'CollPhyForceShakeWave_dalianligongda.webp' }
                ]
            ],
            [ //four
                [
                    { name: '材料力学', school: '西安交通大学', teacher: '殷民', progress: 2, end: false, hot: 140, img: 'MaterForce_xijiaoda.webp' },
                    { name: '工程力学', school: '华中科技大学', teacher: '王元勋', progress: 2, end: false, hot: 100, img: 'ProjForce_huakeda.webp' }
                ],
                [
                    { name: '材料科学基础', school: '大连理工大学', teacher: '赵杰', progress: 2, end: false, hot: 140, img: 'MaterScience_dalianligongda.webp' },
                    { name: '化工原理（下）', school: '北京化工大学', teacher: '丁忠伟', progress: 2, end: false, hot: 100, img: 'ChemiSciencePlus_beijinghuagongda.webp' }
                ]
            ],
        ];
    ...
    function createTab() {
        var Obj = [
            ["计算机", "理学", "工学", "经济管理", "文史哲", "艺术设计", "心理学", "医药卫生", "法学", "教育教学", "农林园艺"],
            ["前言技术", "程序设计与开发", "C语言不挂科", "计算机基础与应用", "软件工程", "网络与安全技术", "硬软件系统及原理", "408计算机", "2021考研计算机", "Python数据分析", "小白编程"],
            ["听力/外语", "语言/阅读", "写作/翻译", "文学与语言学", "专门用途英语", "综合语种", "跨文化交际", "对外汉语", "大学英语", "考神四六级", "考研英语", "考神考研英语", "万词班", "实用英语", "考神雅思", "康哥四六级", "BEC/新概念", "考神专四专八", "专四专八"],
            ["数学", "物理", "化学", "天文学", "地理科学", "生物科学", "大气与海洋", "数理不挂科", "21考研数学"],
            ['电气信息', '工学不挂科', '机械', '土建水利', '力学', '材料', '交通运输', '化工与生物制药', '能源矿业', '轻纺食品', '航天航空', '农林环境', '安全', '21考研机械'],
            ['数学', '唐迟英语', '英语', '政治', '408计算机', '艺术设计', '高数基础', '计算机院校走向', '电子信息', '考神考研', '管理类联考', '专业课1对1'],
            ['数学不挂科', '高数高分', '物理不挂科', '工学不挂科', 'C语言不挂科', '有机化学', '期末高分', '超值组合', '期末试卷解析'],
            ['四六级', '考研英语', '万词班', '雅思极速', '专四专八'],
            ['万词班', '新概念', 'BEC商务英语', '听说读写单项'],
            ['经济', '金融', '电商与贸易', '会计', '管理', '图书情报', '创新创分', '思维力', '实用英语', "金融考证", "管理类联考"],
            ['心理学', '21考研心理学'],
            ['文学文化', '新闻联播', '哲学', '历史', '公务员考试'],
            ['考研设计', '艺术学', '美术学', '戏剧与影视', '设计学', '音乐与舞蹈', '设计教程'],
            ['基础医学', '临床医学', '公共卫生', '口腔医学', '中医中药学', '药学', '中医考研', '护理学', '健康', "老年医学", "儿科学", '妇产科学', '急诊与灾害医学', '医患沟通与伦理学'],
            ['教学方法', '教学能力', '信息化教学', '职业素养', '学科教学', '素质教育', '体育教育', '学前教育', '考研教育学', "公务员考试"],
            ['政治', '法学', '思政', '社会'],
            ['植物', '动物', '生态'],
            [''],
            ['数学', '孔昱力政治', '王一珉政治', '唐迟英语', '康哥英语', '408计算机', '考研设计', '猴博士高数', '21考研机械', "21考研经济学", "21考研心理学", '西医综合', '管理联考', '农科化学', '考研教育学', '中医考研', '领学班', '更多专业课'],
            ['Python数据分析', '小白编程', '办公技能', '设计教程', '金融考证', '公务员考试', '计算机二级', '教师资格证', '教师招聘', "记忆力"],
            ['热门专栏', '讲座Live', '效率', '职场实用技能', '情商', '思维力', '外语', '爱情', '钢琴', "人文", "二胡", "其他"],
            [],
            ['公共卫生', '护理学', '健康'],
            ['小语种', '摄影', '剪辑后期', '办公技能', '穿搭美妆', '手工制作', '美食烹饪', '锻炼健身', '配音写作', '绘画', '乐器', '表演']

        ];
        var Title = ["国家精品", "计算机", "外语", "理学", "工学", "22考研", "期末不挂科", "应试英语", "实用英语", "经济管理", "心理学",
            "文史哲", "艺术设计", "医药卫生", "教育教学", "法学", "农林园艺", "音乐与舞蹈", "21考研", "考证就业", "名师专栏", "体育运动",
            "养生保健", "兴趣爱好"
        ];
        for (let i = 0; i < 23; i++) {
            channeltabDB.create({
                    ...
                    title: Title[i],
                    list: Obj[i],
                    show: (i == 20 || i == 21 || i == 22) ? false : true
                }).then(d => {})
                .catch(e => { console.log(e); })
        };
    };

    function createHeaderCourse() {
        for (let i = 0; i < headercourse.length; i++) {
            let plist = [];
            for (let j = 0; j < headercourseList[i].list.length; j++) {
                plist.push({
                    title: headercourseList[i].list[j].title,
                    channel: headercourseList[i].list[j].channel
                });
            };
            headercourseDB.create({
                    title: headercourseList[i].title,
                    list: plist
                }).then(data => {})
                .catch(e => { console.log("createHeaderCourse--catch:" + e) });
        }
    };
